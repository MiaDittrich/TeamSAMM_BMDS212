#!/usr/bin/env python3
"""
BRCA1 MAVE Score Prediction Pipeline v2 — Improved
====================================================
...
--------------------
1. ClinVar hold-out:   20 variants (10 pathogenic / 10 benign) are withheld
                       from all training; predicted after CV, then classified.
2. KNN imputation:     KNNImputer (k=5) replaces median imputation, using
                       structural neighbours to fill missing AlphaMissense values.
3. Feature engineering: three derived features added —
                        am_pathogenicity² (non-linear AlphaMissense signal),
                        am × EvoEF2 interaction, pLDDT-weighted CA displacement.
4. Stacking ensemble:  RF + XGB + LightGBM → Ridge meta-learner.
5. Wider tuning grids: 7-point alpha grid for Ridge; 3×3 grid for RF.
6. Bootstrap CIs:      500-resample bootstrap confidence intervals for every
                       ClinVar prediction.
7. Classification:     AUROC, balanced accuracy, sensitivity, specificity and
                       ROC curve on the 20-variant ClinVar test set.

ClinVar held-out set
--------------------
All 20 variants confirmed present in BOTH BRCA1_FEATS and BRCA1_MAVE
(every variant therefore has a valid am_pathogenicity score).

Pathogenic (10): S1841R L1839S V1838E M1775R Y1703S W1718L
                 G1706R G1738E S1715R I1760S
Benign (10)    : D1733G I1766V T1773S V1736I K1793Q E1794G S1797C
                 H1862L P1831S E1829T

Usage
-----
python brca1_mave_pipeline_v2.py
python brca1_mave_pipeline_v2.py --model xgb
python brca1_mave_pipeline_v2.py --no-ablation --n-bootstrap 200
python brca1_mave_pipeline_v2.py --labels both --model stack

Output
------
All plots and CSVs → --outdir (default: brca1_results_v2/).
"""
...
# ClinVar held-out test set (withheld from ALL training)
# All 20 variants are confirmed present in BOTH BRCA1_FEATS and BRCA1_MAVE,
# so every variant has a valid am_pathogenicity score.
#
# Original picks A1708E, R1699Q, M1775K, W1718S (path) and V1665M, D1692N,
# M1775I (benign) were absent from BRCA1_MAVE and have been replaced:
#   G1706R  — Gly1706Arg,  ClinVar Pathogenic
#   G1738E  — Gly1738Glu,  ClinVar Pathogenic
#   S1715R  — Ser1715Arg,  ClinVar Pathogenic (rs80357382)
#   I1760S  — Ile1760Ser,  ClinVar Pathogenic/Likely Pathogenic
#   H1862L  — His1862Leu,  ClinVar Benign
#   P1831S  — Pro1831Ser,  ClinVar Benign/Likely Benign
#   E1829T  — Glu1829Thr,  ClinVar Benign
CLINVAR_PATHOGENIC = [
...
FEATURE_GROUPS = {
    "structural": [
        ...
        "ramachandran_violation", "rsa", "ss_helix", "ss_strand", "ss_coil",
...
def preprocess_features(df: pd.DataFrame, engineer: bool = True):
    """
    Encodes am_class, drops all-NaN features, optionally adds 3 engineered
    features.  Applied once to the full merged dataframe (all 178 variants)
    so the ClinVar rows share the same feature space.
    """
    df = df.copy()
    df["am_class_enc"] = df["am_class"].map(AM_CLASS_MAP)

    # Collect base features in declared order
    base_features = []
    ...
    eng_features: list[str] = []
    if engineer:
        # 1. Squared AlphaMissense pathogenicity — captures non-linearity near 1.0
        if "am_pathogenicity" in base_features:
            df["am_pathogenicity_sq"] = df["am_pathogenicity"] ** 2
            eng_features.append("am_pathogenicity_sq")

        # 2. AlphaMissense × log-EvoEF2 interaction
        if "am_pathogenicity" in base_features and "evoef2_ddg_Total" in base_features:
            ...
            eng_features.append("am_x_evo")

        # 3. pLDDT-weighted CA displacement (confidence × structural disruption)
        if "mutant_plddt" in base_features and "ca_rmsd" in base_features:
            df["plddt_rmsd"] = (df["mutant_plddt"] / 100.0) * df["ca_rmsd"]
            eng_features.append("plddt_rmsd")
...
def clinvar_split(label_sets: dict, all_features: list, merged_df: pd.DataFrame):
    ...
    Returns
    -------
    train_label_sets : dict — same structure as label_sets but ClinVar rows removed
    clinvar_df       : DataFrame — 20 ClinVar variants with features + clinvar_label
    """
...
def get_models_v2(seed: int = 42, tune: bool = True) -> dict:
    """
    Improved pipeline.
    - KNNImputer(k=5) instead of median
    - Wider tuning grids for ridge and RF
    - Stacking ensemble: RF + XGB + LightGBM → Ridge meta
    """
    inner = KFold(n_splits=3, shuffle=True, random_state=seed)
    ...
    # Stacking base estimators (fixed hyperparameters — no further tuning to keep
    # the inner CV of StackingRegressor from becoming prohibitively slow on n≈70)
    stack_base = [
...
def make_bootstrap_pipe(fitted_model_pipe) -> Pipeline:
    """
    Build a fast bootstrap-ready pipeline from an already-fitted pipeline.

    Any GridSearchCV step is replaced by its best_estimator_ (fixed
    hyperparameters).  This means bootstrap resamples skip the inner CV
    entirely — hyperparameter selection was already done on the full training
    set, so repeating it on every bootstrap resample is unnecessary and would
    take ~27× longer per resample.
    """
    new_steps = []
...
def bootstrap_predict(model_pipe, X_train: pd.DataFrame, y_train: pd.Series,
                       ...
                       seed: int = 42) -> dict:
    """
    Train model on n_bootstrap resamples of (X_train, y_train), predict X_test.

    Pass a pipeline WITHOUT GridSearchCV (use make_bootstrap_pipe first) so
    that each resample only fits the final estimator — not an inner CV grid.

    Returns
    -------
    dict with keys: mean, median, ci_lo (2.5%), ci_hi (97.5%), std, all (n_bs × n_test)
    """
...
def evaluate_clinvar_classification(clinvar_df: pd.DataFrame,
                                     ...
                                     model_name: str) -> dict:
    """
    Classify ClinVar variants and report AUROC, accuracy, sensitivity, specificity.

    Labelling convention: 1 = pathogenic, 0 = benign.
    Predicted score < train_median → classified as pathogenic.
    """
    ...
    # AUROC uses continuous score (negated so higher damage_score → pathogenic)
    damage_score = -y_mean
    ...
    # Hard classification at training-set median
...
def plot_improvement_comparison(results_v1: dict, results_v2: dict, outdir: Path):
    """Side-by-side bar chart: v1 original vs v2 improved (shared models only)."""
    ...
    for col, (metric, mlabel) in enumerate(zip(metrics, metric_labels)):
        ...
        ax.bar(x + w/2, v2_m, w, yerr=v2_s, label="v2 — improved",
    ...
    fig.suptitle(
        "v1 (Original) vs v2 (Improved)  —  Repeated 5-Fold CV on Training Set\n"
        "KNN Imputation + Feature Engineering + Wider Grids  |  Green = improvement",
        fontsize=11, fontweight="bold",
    )
    plt.tight_layout()
...
def plot_model_comparison(results_all: dict, outdir: Path, suffix: str = "v2"):
    ...
    fig.suptitle(f"Model Comparison ({suffix})  —  Repeated K-Fold CV, mean ± std",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def plot_cv_distributions(results_all: dict, outdir: Path, suffix: str = "v2"):
    ...
    for col, lname in enumerate(label_names):
        ...
        ax.axhline(0, color="#aaa", linestyle="--", linewidth=0.8)
    fig.suptitle(f"Cross-Validation R² Distributions ({suffix})",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def plot_predictions_scatter(results_all: dict, label_sets: dict,
    ...
    fig.suptitle(f"OOF Predicted vs Actual — {best_model.upper()} (v2)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def plot_feature_importance(model_pipe, feature_names: list,
    ...
    # For stacking, aggregate importances across base models
...
def plot_feature_ablation(abl_df: pd.DataFrame, label_name: str,
    ...
    ax2.axvline(abl_df["baseline_r2"].iloc[0], color="#333", linestyle="--",
                linewidth=1.2)
...
def plot_group_ablation(abl_df: pd.DataFrame, label_name: str,
    ...
    for ax, metric, ylabel in zip(axes, ["delta_r2", "delta_pearson"], ["ΔR²", "ΔPearson r"]):
    ...
    fig.suptitle(f"Feature Group Ablation  |  baseline R²={abl_df['baseline_r2'].iloc[0]:.3f}",
...
def plot_best_model_heatmap(summary_df: pd.DataFrame, outdir: Path):
    ...
    fig.suptitle("Model × Label Strategy Performance Overview (v2)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    _save(fig, outdir / "10_best_model_heatmap.png")


# ── ClinVar-specific plots ────────────────────────────────────────────────────

def plot_clinvar_predictions(clf_result: dict, label_strategy: str,
                              model_name: str, outdir: Path):
    """Predicted MAVE scores with 95% CI, colored by ClinVar label."""
...
def plot_clinvar_roc(clf_results_all: dict, outdir: Path, model_name: str = ""):
    """ROC curves for all label strategies."""
...
def plot_clinvar_summary_heatmap(clf_results_all: dict, outdir: Path, model_name: str = ""):
    """Heatmap of classification metrics across label strategies."""
    rows = []
    for lname, res in clf_results_all.items():
        rows.append({
            "label":              lname,
...
def parse_args():
    ...
    p.add_argument("--feats",   default="BRCA1_FEATS.csv")
    p.add_argument("--mave",    default="BRCA1_MAVE.csv")
...
def main():
    ...
    print(f"\n{'═' * 62}")
    print(f"  BRCA1 MAVE Prediction Pipeline  v2 — Improved")
    print(f"{'═' * 62}")
    ...
    # ── 2. Preprocess (applies feature engineering to all 178 rows) ───────────
    ...
    # Filter to requested label strategies
    if args.labels != "all":
    ...
    # ── 6. Baseline (v1) evaluation ───────────────────────────────────────────
    results_v1 = {}
    if args.run_baseline:
        banner("STEP 4 — Baseline (v1) cross-validated evaluation")
    ...
    for lname, ldf in train_label_sets.items():
        ...
        # Fit model once on full training data (gets best hyperparameters).
        # Then strip GridSearchCV so bootstrap resamples don't re-run inner CV.
        final_model = clone(all_models_v2[best_model_name])
        final_model.fit(X_tr, y_tr)
        bootstrap_pipe = make_bootstrap_pipe(final_model)

        # Bootstrap CIs (fast: fixed hyperparameters, no inner CV per resample)
    ...
    # ── 11. Ablation ─────────────────────────────────────────────────────────
    if args.ablation:
        banner("STEP 8 — Ablation studies")
        best_label_name = (max if higher_better else min)(
            results_v2,
            key=lambda l: _scalar(results_v2[l][best_model_name], rm)
            if best_model_name in results_v2[l] else (float("-inf") if higher_better else float("inf")),
        )
        ...
        print("\n  Top features by ΔR²:")
        print(abl_feat[["feature", "group", "delta_r2", "r2_without"]].head(10).to_string(index=False))
        print("\n  Group ablation:")
        print(abl_grp[["group", "n_features", "delta_r2", "delta_pearson", "r2_without"]].to_string(index=False))

    # ── 12. Best combo summary ────────────────────────────────────────────────
    if args.find_best:
        banner("STEP 9 — Best model × label selection (v2)")
